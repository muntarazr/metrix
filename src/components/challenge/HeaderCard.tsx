...
export function HeaderCard({
  ...
  const shellClass =
    'overflow-hidden border-border bg-card shadow-sm shadow-black/[0.02]';
  const innerPanelClass =
    cn(
      'mt-3 rounded-xl border border-border/70 bg-background',
    ...
    <section className={cn(!embedded && cardClass, !embedded && shellClass)}>
        ...
        <div className={innerPanelClass}>
          ...
          <div className="mt-2 flex flex-col gap-2 sm:flex-row">
            <input
              ...
              maxLength={8}
              className="h-10 flex-1 rounded-xl border border-border bg-card px-3 py-2.5 text-sm font-mono tracking-[0.18em] text-foreground outline-none focus:ring-1 focus:ring-primary/25"
              dir="ltr"
            />
            <button
              onClick={onJoin}
              disabled={busyAction !== null || joinCode.trim().length < 8}
                  className="flex h-10 shrink-0 items-center justify-center gap-2 rounded-lg border border-border bg-muted px-3 text-xs font-semibold text-muted-foreground transition-colors hover:opacity-90 active:scale-[0.98] disabled:opacity-60"
            >
      ...
      {snapshot.status === 'pending' && (
        <div className={innerPanelClass}>
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              className="flex h-10 shrink-0 items-center justify-center gap-2 rounded-lg border border-border bg-muted px-3 text-xs font-semibold text-muted-foreground transition-colors hover:opacity-90 active:scale-[0.98] disabled:opacity-60"
                ...
                <button
                  ...
                  disabled={busyAction !== null}
                  className="flex h-10 shrink-0 items-center justify-center gap-1.5 rounded-lg border border-destructive/25 bg-destructive/12 px-3 text-xs font-semibold text-destructive transition-colors hover:opacity-90 active:scale-[0.98] disabled:opacity-50"
                >
          ...
          {confirmCancel && (
            <div className="mt-2 rounded-xl border border-destructive/25 bg-destructive/12 px-3 py-2.5">
              ...
              <div className="mt-3 flex flex-col gap-2 sm:flex-row">
                ...
                <button
                  ...
                  className="flex h-8 items-center justify-center rounded-lg border border-border bg-muted px-3 text-xs font-semibold text-muted-foreground transition-colors hover:opacity-90 active:scale-[0.98]"
                >
          ...
          {!confirmCancel && (
            <div className="mt-2 rounded-xl border border-dashed border-border/70 bg-muted/20 px-3 py-2.5 text-xs leading-5 text-muted-foreground">
              {ui.inviteReady}
            </div>
          )}
        </div>
      ...
      {snapshot.status === 'active' && (
        <div className={innerPanelClass}>
          <div className={cn('grid gap-2', embedded ? 'grid-cols-2' : 'grid-cols-1 min-[420px]:grid-cols-3')}>
            ...
            <MetricBox label={ui.total} value={numberFormatter.format(snapshot.scoreboard.opponent.total)} toneClass="text-primary" />
          ...
          <div className={cn('flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between', embedded ? 'mt-2.5' : 'mt-3')}>
            <div className="inline-flex items-center gap-2 rounded-lg border border-border/70 bg-muted/20 px-3 py-2 text-xs text-muted-foreground">
            ...
            <button
              ...
              disabled={busyAction !== null}
              className="flex h-10 items-center justify-center gap-2 rounded-xl border border-destructive/25 bg-destructive/12 px-3 text-xs font-semibold text-destructive transition-colors hover:opacity-90 active:scale-[0.98] disabled:opacity-60"
            >
