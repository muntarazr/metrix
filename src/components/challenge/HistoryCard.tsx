...
export function HistoryCard({ history, historyLoading, locale, ui, t, embedded = false }: HistoryCardProps) {
  ...
  const innerPanelClass =
    'mt-3 rounded-xl border border-border/70 bg-background p-3 sm:p-4';
      ...
      <section className={cn(!embedded && cardClass, !embedded && shellClass)}>
        ...
        >
          <div className="flex min-w-0 items-center gap-3">
            <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-[10px] bg-muted/60 text-muted-foreground border border-border/70">
  ...
  return (
    <section className={cn(!embedded && cardClass, !embedded && shellClass)}>
      ...
      >
        <div className="flex min-w-0 items-center gap-3">
          <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-muted/60 text-muted-foreground ring-1 ring-border/45">
            <History className="h-4 w-4" />
          </div>
          <div className="min-w-0 flex-1">
        ...
        <div className="flex shrink-0 items-center gap-2">
          <span className="inline-flex h-fit items-center rounded-full border border-border/70 bg-muted/20 px-2.5 py-1 text-[10px] font-semibold text-muted-foreground">
            {history.length}
          </span>
          ...
          <div className="space-y-2 max-h-[22rem] overflow-y-auto pr-0.5 scrollbar-thin sm:max-h-[24rem] lg:max-h-[26rem]">
            {history.map((item) => (
              <div
                ...
                className={cn(
                  softCardClass,
                  'rounded-xl border-border/70 bg-background px-3 py-2.5',
                )}
              >
