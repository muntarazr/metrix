...
export default function ActivityHistory({ logs, language = 'ar', onLogDeleted, onLogProgress, embedded = false }: ActivityHistoryProps) {
    ...
    if (logs.length === 0) {
        return (
            <div className={embedded ? "h-full" : cn(PANEL_SURFACE, "p-4 sm:p-8 rounded-[20px] sm:rounded-[32px]")}>
    ...
    return (
        <div
            className={embedded ? "h-full min-h-0 flex flex-col overflow-hidden" : cn(PANEL_SURFACE, "p-4 sm:p-6 rounded-[20px] sm:rounded-[32px] relative overflow-hidden flex flex-col h-full min-h-[280px] max-h-[56vh]")}
        ...
        >
            ...
            <div className="shrink-0 px-1 sm:px-1.5 pb-2 sm:pb-3">
                <div className="flex items-center justify-between gap-2 sm:gap-3 h-11 sm:h-12 rounded-xl border border-border bg-background px-2.5 sm:px-3">
            ...
            <div
                className={embedded
                    ? "flex-1 min-h-0 overflow-y-auto px-1 sm:px-1.5 pb-2 sm:pb-3 space-y-2.5 sm:space-y-3 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-muted/20 [&::-webkit-scrollbar-thumb]:rounded-full hover:[&::-webkit-scrollbar-thumb]:bg-muted/60 transition-colors"
                    : "flex-1 min-h-0 overflow-y-auto px-1 sm:px-1.5 pb-3 sm:pb-4 space-y-2.5 sm:space-y-3 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-muted/20 [&::-webkit-scrollbar-thumb]:rounded-full hover:[&::-webkit-scrollbar-thumb]:bg-muted/60 transition-colors"
                }
            >
                ...
                {logs.map((log) => {
                    ...
                    return (
                        <div key={log.id} className={cn(PANEL_SURFACE, "p-3 sm:p-4 rounded-2xl transition-colors group hover:border-border", cardToneClass)}>
