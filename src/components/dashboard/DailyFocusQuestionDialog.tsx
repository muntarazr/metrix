...
export default function DailyFocusQuestionDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        {/*
          Height tracks the viewport instead of a fixed ceiling. The old desktop
          cap was min(75vh,34rem) — 34rem (544px) wins on any screen taller than
          ~725px, so the dialog stayed the same small box on a large monitor. The
          rem term is now high enough that svh is what actually binds, and it
          only takes over on a very tall screen where a narrow full-height modal
          would look wrong.
        */}
        <div className="flex max-h-[88svh] flex-col sm:max-h-[min(88svh,52rem)]">
          {/* Single custom close button — logical inset for LTR/RTL */}
          <DialogClose className="absolute top-3 end-3 z-10 flex size-7 items-center justify-center rounded-full bg-muted/60 text-muted-foreground transition-colors hover:bg-muted hover:text-foreground focus:outline-none sm:top-4 sm:end-4">
            <X className="size-4" />
          ...
          <div className="flex-1 overflow-y-auto pb-4">
            ...
            <div className="px-5 pt-4 pb-3 pe-12 sm:px-7 sm:pt-5 sm:pb-4 sm:pe-14">
              ...
              >
                {/* Badge row: logical separator works in both directions */}
                  ...
                  >
                  <span className="inline-flex items-center gap-1.5 rounded-full bg-primary/8 px-3 py-1.5 text-[11px] font-bold uppercase tracking-wide text-primary dark:bg-primary/20">
                    <Sparkles className="size-3.5 shrink-0" aria-hidden />
                    {t.dailyFocus}
                  </span>
                  <span className="text-[12px] font-semibold text-muted-foreground/75 ms-0.5 border-s border-border ps-2.5 dark:border-white/10">
                    {cycleLabel}
                  </span>
                </div>
                    ...
                    <div className={cn("mt-3 rounded-xl border border-primary/15 bg-primary/12 dark:bg-white/5 sm:rounded-2xl")}>
                      <button
                        ...
                        className={cn(
                          ...
                          "text-[10px] font-bold uppercase tracking-wider text-primary/75 sm:text-[11px]",
            ...
            <div className="px-5 pt-3 pb-2 sm:px-7">
              <label className="mb-1.5 block text-start text-[10px] font-bold uppercase tracking-[0.18em] text-muted-foreground/90 sm:text-[11px]">
                {t.answerQuestion}
              </label>
          ...
          {/* ── Footer ── */}
          <div className="border-t border-border/70 bg-background px-5 py-3 sm:px-7 sm:py-4">
            <button
              type="button"
              onClick={onSubmit}
