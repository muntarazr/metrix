...
interface DayCalendarGridProps {
  ...
  /** Points achievable in a single day — the denominator behind every cell colour. */
  dailyCap: number;
  language?: Language;
  loading?: boolean;
...
/**
 * Heat is measured against the points that were actually achievable that day
 * (the goal's daily cap), never against the month's own peak. A month holding
 * one zero-point entry made that entry the peak, so it painted at full
 * intensity — the darkest day and the emptiest day looked identical.
 */
type HeatTier = 0 | 1 | 2 | 3 | 4 | 5;
...
const HEAT_SURFACE: Record<HeatTier, string> = {
  ...
  4: "border-transparent bg-primary/60",
  5: "border-transparent bg-primary/90",
};

/**
 * Dark on every tier. The teal primary tops out around rgb(24,146,177), which
 * is still light enough that near-black beats white at the darkest step
 * (5.45:1 against 3.63:1) — measured, not assumed. Switching to white at high
 * intensity, as this grid used to, made the busiest days the hardest to read.
 */
const HEAT_TEXT: Record<HeatTier, string> = {
...
/** Today stays marked whether or not it has been logged, on light and dark tiers alike. */
const HEAT_TODAY_RING: Record<HeatTier, string> = {
  0: "ring-2 ring-primary/45 ring-inset",
  1: "ring-2 ring-primary/45 ring-inset",
  2: "ring-2 ring-primary/70 ring-inset",
  3: "ring-2 ring-foreground/45 ring-inset",
  4: "ring-2 ring-foreground/45 ring-inset",
  5: "ring-2 ring-foreground/45 ring-inset",
};
...
function heatTier(
  ...
  if (!hasLogs) return 0;
  // Logged, but nothing scored — its own state, not the strongest day of the month.
  if (points <= 0) return 1;

  const ratio = dailyCap > 0 ? points / dailyCap : 0;
...
export default function DayCalendarGrid({
  ...
  if (loading) {
    return (
      <div className={cn("relative flex h-full w-full flex-col overflow-hidden rounded-2xl p-1.5 sm:p-2.5", PANEL_SURFACE)}>
        <div className={cn("rounded-xl p-1 sm:p-1.5", WELL_SURFACE)}>
          <div className="flex items-center gap-0.5" dir="ltr">
            <div className="h-7 w-7 shrink-0 rounded-lg bg-muted/60 animate-pulse" />
            <div className="mx-auto h-4 w-24 rounded-md bg-muted/60 animate-pulse" />
            <div className="h-7 w-7 shrink-0 rounded-lg bg-muted/60 animate-pulse" />
          </div>
        </div>
        <div className="mt-1 flex flex-1 flex-col sm:mt-1.5">
          <div className="mb-1 grid grid-cols-7 gap-0.5 rounded-lg border border-border/45 bg-muted/12 p-0.5 sm:mb-1.5 sm:gap-1">
            {Array.from({ length: 7 }).map((_, i) => (
              <div key={i} className="h-5 rounded-md bg-muted/20 animate-pulse sm:h-6" />
            ))}
          </div>
  ...
  return (
    <>
      <div
        className={cn(
          "relative flex h-full w-full flex-col overflow-hidden rounded-2xl p-1.5 sm:p-2.5",
          PANEL_SURFACE,
        )}
      ...
      >
          ...
          <div className="flex items-center gap-1" dir="ltr">
            ...
            >
              <PopoverTrigger asChild>
                <button
                  type="button"
                  className="group flex min-w-0 flex-1 items-center justify-center gap-1 rounded-lg px-1 py-1 text-center transition-colors hover:bg-muted/20 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/45 focus-visible:ring-offset-2 focus-visible:ring-offset-card"
              ...
              <PopoverContent
                ...
                className="w-[17rem] rounded-xl border-border/70 bg-background/90 p-3 shadow-xl backdrop-blur"
                dir={isArabic ? "rtl" : "ltr"}
              >
                <div className="space-y-3">
                  <div className="flex items-center justify-between gap-2">
                    <button
                      ...
                      className="flex h-8 w-8 items-center justify-center rounded-lg border border-border text-muted-foreground transition-colors hover:bg-muted/60 hover:text-foreground"
                      ...
                      <button
                        ...
                        className={cn(
                          ...
                          item.isActive
                            ...
                            : "border border-border bg-muted/20 text-muted-foreground hover:bg-muted/60 hover:text-foreground",
                        )}
                      >
                        {item.label}
                      </button>
        ...
        <div className="mt-1 flex flex-1 flex-col sm:mt-1.5">
          <div
            className={cn("mb-1 grid grid-cols-7 gap-0.5 rounded-lg p-0.5 sm:mb-1.5 sm:gap-1", WELL_SURFACE)}
            dir="ltr"
          >
                ...
                <div
                  ...
                  className={cn(
                    ...
                    isCurrentWeekday
                      ...
                      : "border-transparent bg-card text-muted-foreground/75",
                  )}
                >
                  {dayLabel}
                </div>
          ...
          <div className="grid grid-cols-7 gap-0.5 sm:gap-1" dir="ltr">
            {calendarCells.map((day, index) => {
              ...
              const tier = heatTier(day.totalPoints, dailyCap, day.hasLogs);
                ...
                <button
                  ...
                  className={cn(
                    ...
                    day.isToday &&
                      !isSelected &&
                      !day.isBeforeStart &&
                      cn(HEAT_TODAY_RING[tier], "shadow-sm"),
                    isSelectable &&
                      "hover:ring-2 hover:ring-primary/45 hover:ring-inset active:scale-95",
                    isSelected &&
                      "scale-[1.02] brightness-95 ring-2 ring-primary/25 ring-inset",
                    day.badge === "strong" &&
                      !day.isBeforeStart &&
                      !day.isAfterToday &&
                    ...
                    <span
                      className={cn(
                        ...
                        day.badge === "exceptional"
                          ...
                          : day.badge === "strong"
                            ? "bg-chart-2"
                            : "bg-foreground/60 dark:bg-foreground/60",
                      )}
                    />
                  )}
                </button>
        ...
        <div className="mt-1 flex items-center justify-between gap-1.5 overflow-hidden pt-1 sm:mt-1.5 sm:gap-2 sm:pt-1.5">
          <div className="min-w-0 flex flex-1 items-center overflow-hidden">
            <div className="inline-flex max-w-full items-center gap-1 overflow-hidden rounded-full bg-muted/12 px-1.5 py-0.5 text-[10px] font-medium leading-none whitespace-nowrap sm:gap-1.5 sm:px-2.5 sm:py-1">
      ...
      {tooltip && (
        <div
          className="pointer-events-none fixed z-[200] whitespace-nowrap rounded-xl border border-border/70 bg-popover px-3 py-2 text-xs text-popover-foreground shadow-lg shadow-black/10"
          style={{
            left: tooltip.x,
            top: tooltip.y,
        ...
        >
          {tooltip.state === "before-start"
            ...
            : tooltip.state === "future"
              ...
              : tooltip.count > 0
                ? `${formatNumberEn(tooltip.count)} ${labels.logs} · ${formatNumberEn(tooltip.pts)}/${formatNumberEn(dailyCap)} ${labels.points}${tooltip.badgeLabel ? ` · ${tooltip.badgeLabel}` : ""} — ${tooltip.date}`
      ...
      {selectedDate && typeof document !== "undefined" && createPortal(
        ...
        >
          <div
            className="relative flex max-h-[calc(100dvh-0.75rem-env(safe-area-inset-bottom))] w-full max-w-none flex-col overflow-hidden rounded-t-[24px] border border-border bg-card shadow-2xl shadow-black/10 sm:max-h-[85vh] sm:w-[min(calc(100vw-2rem),28rem)] sm:rounded-[24px] lg:w-[min(calc(100vw-3rem),42rem)] lg:max-w-2xl lg:rounded-2xl"
          ...
          >
            ...
            <div className="flex items-start justify-between gap-4 border-b border-border/70 px-4 pb-3 sm:px-5 sm:pb-4">
              <div>
                <p className="text-[11px] font-bold text-muted-foreground/75 uppercase tracking-wider">
                  {labels.dayDetail}
                </p>
              ...
              <div className="flex shrink-0 items-center gap-2.5 pt-1">
                {selectedLogs.length > 0 && (
                  <div className="flex items-center gap-1.5 rounded-xl bg-primary/12 px-3 py-1.5 text-xs font-bold text-primary ring-1 ring-primary/25 shadow-sm">
                ...
                selectedLogs
                  ...
                  .map((log) => {
                    ...
                    return (
                      <div
                        key={log.id}
                        className="space-y-2.5 rounded-xl border border-border/70 bg-muted/12 p-4 shadow-sm transition-all duration-200 hover:border-border hover:shadow-md"
                      >
                        <div className="flex items-center justify-between gap-2">
                          ...
                          <div className="flex items-center gap-1.5 rounded-xl bg-primary/12 px-2.5 py-1 text-xs font-bold text-primary ring-1 ring-primary/25 shadow-sm">
                              ...
                              performanceMeta?.performance_tier ===
                                ...
                                : performanceMeta?.performance_tier === "strong"
                                  ...
                                  : performanceMeta?.performance_tier === "weak"
                                    ...
                                    : "bg-foreground/12 text-foreground ring-foreground/25",
                            )}
                          >
                            {performanceLabel}
                          </div>
                        ...
                        {log.ai_feedback && (
                          <div className="flex items-start gap-2.5 border-t border-border/45 pt-2.5 text-xs text-muted-foreground/75">
