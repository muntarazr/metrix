...
export default function GoalCompletionCelebration({ goalTitle, language = 'ar', onClose }: GoalCompletionCelebrationProps) {
    ...
    return (
        ...
        >
            ...
            {showConfetti && (
                <div className="absolute inset-0 pointer-events-none flex justify-center">
                    <ConfettiBoom
                        ...
                        spreadDeg={60}
                        launchSpeed={1.4}
                        colors={['#0097b2', '#18181b', '#52525b', '#a1a1aa', '#d4d4d8', '#ffffff']}
                    />
                </div>
            )}
            ...
            >
                {/* Gradient header */}
                <div className="bg-gradient-to-br from-primary/20 via-primary/20 to-chart-1/20 p-8 text-center relative overflow-hidden">
                    <div className="absolute inset-0 bg-gradient-to-b from-transparent to-card/20" />

                    {/* Stars background */}
                    <Star className="absolute top-3 left-5 w-4 h-4 text-primary/50 animate-pulse" />
                    <Star className="absolute top-6 right-8 w-3 h-3 text-chart-1/40 animate-pulse delay-150" />
                    <Sparkles className="absolute bottom-4 left-8 w-4 h-4 text-primary/50 animate-pulse delay-300" />
                    <Sparkles className="absolute bottom-3 right-5 w-3 h-3 text-primary/50 animate-pulse delay-100" />

                    <div className="relative z-10">
                        <div
                            className="w-24 h-24 mx-auto mb-4 rounded-full flex items-center justify-center shadow-xl"
                            style={{
                                background: 'linear-gradient(135deg, var(--chart-5), var(--chart-1))',
                ...
                <div className="px-6 pb-6">
                    <button
                        ...
                        style={{ background: 'linear-gradient(135deg, var(--chart-5), var(--primary))' }}
                    >
