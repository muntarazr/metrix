...
export default function GoalCreator({
    ...
    const renderNotification = () => (
        notification ? (
            <Alert
                ...
                className={cn(
                    "mb-4",
                    notification.type === 'warning' && 'border-foreground/25 text-foreground [&>svg]:text-foreground/75',
                    notification.type === 'info' && 'border-primary/25 text-primary [&>svg]:text-primary',
                )}
            >
                ...
                <Collapsible open={isStructuredOpen} onOpenChange={setIsStructuredOpen}>
                    <Card className="py-0 overflow-hidden">
                        <CollapsibleTrigger asChild>
                            <button className="w-full flex items-center justify-between px-5 py-3.5 hover:bg-accent/60 transition-colors">
                                                ...
                                                <AccordionContent className="space-y-4 pb-4">
                                                    ...
                                                    <div className="space-y-3">
                                                        ...
                                                        {main.subtasks.map((sub) => (
                                                            <Card key={sub.id} className="py-0 gap-0 bg-muted/20">
    ...
    if (step === 'QUESTIONS' && investigationResult) {
        return (
            <div className="space-y-5">
                ...
                <Alert className="border-primary/25 bg-primary/12">
                ...
                <Card className="overflow-hidden border-border bg-gradient-to-br from-card via-card to-muted/20 shadow-md">
                    <CardHeader className="border-b border-border bg-muted/20 px-6 py-4">
                        ...
                        <CardTitle className="flex items-center gap-2 text-base sm:text-lg mb-1">
                            <span className="flex size-8 items-center justify-center rounded-xl bg-primary/8 text-primary">
    ...
    return (
        <div className="space-y-5">
            ...
            <Card className="overflow-hidden border-primary/25 bg-gradient-to-br from-card/60 via-card/60 to-primary/12 backdrop-blur-md shadow-lg transition-all duration-300 hover:shadow-xl hover:border-primary/25 relative">
                {/* Background decorative glowing element */}
                <div className="absolute top-0 right-0 -mt-6 -mr-6 w-24 h-24 bg-primary/8 rounded-full blur-2xl pointer-events-none" />
                <div className="absolute bottom-0 left-0 -mb-6 -ml-6 w-24 h-24 bg-foreground/12 rounded-full blur-2xl pointer-events-none" />
                ...
                <CardContent className="p-6 space-y-6">
                    ...
                    <div className="flex items-center gap-3">
                        <span className="flex size-9 items-center justify-center rounded-xl bg-primary/12 text-primary border border-primary/25">
                    ...
                    <div className="relative group rounded-2xl bg-muted/20 border border-border/70 p-4 flex flex-col sm:flex-row items-center justify-between gap-4 overflow-hidden">
                        ...
                        <div className="relative flex items-center justify-center min-w-[130px] py-1.5 px-4 rounded-xl bg-gradient-to-r from-primary/12 to-primary/12 border border-primary/25 shadow-[0_0_20px_-5px_color-mix(in_oklch,var(--primary)_30%,transparent)]">
                    ...
                    <div className="space-y-3">
                        <div className="flex items-center justify-between">
                            ...
                            <Badge variant="secondary" className="font-bold text-xs bg-primary/12 text-primary border border-primary/25">
                        ...
                        <div className="flex items-center justify-between text-[11px] text-muted-foreground">
                            ...
                            <span className="flex items-center gap-1.5 font-medium text-foreground bg-muted/60 px-2 py-0.5 rounded-md border border-border/45">
                            ...
                            {(['easy', 'medium', 'hard', 'expert', 'legendary'] as const).map((tier) => {
                                ...
                                const tierMeta = {
                                    easy: {
                                        ...
                                        labelAr: 'سهل',
                                        rate: '50/d',
                                        classes: 'border-foreground/15 text-muted-foreground/75',
                                        activeClasses: 'bg-foreground/60 text-background border-foreground/45'
                                    },
                                    medium: {
                                        labelEn: 'Medium',
                                        labelAr: 'متوسط',
                                        rate: '100/d',
                                        classes: 'border-foreground/15 text-muted-foreground',
                                        activeClasses: 'bg-foreground/60 text-background border-foreground/45'
                                    },
                                    hard: {
                                        labelEn: 'Hard',
                                        labelAr: 'صعب',
                                        rate: '150/d',
                                        classes: 'border-foreground/15 text-foreground/75',
                                        activeClasses: 'bg-foreground/60 text-background border-foreground/70'
                                    },
                                    expert: {
                                        labelEn: 'Expert',
                                        labelAr: 'خبير',
                                        rate: '200/d',
                                        classes: 'border-foreground/25 text-foreground',
                                    ...
                                    legendary: {
                                        ...
                                        labelAr: 'أسطوري',
                                        rate: '250/d',
                                        classes: 'border-primary/25 text-primary',
                                        activeClasses: 'bg-primary text-primary-foreground border-primary shadow-[0_0_12px_color-mix(in_oklch,var(--primary)_35%,transparent)]'
                                    }
                                }[tier];
                                ...
                                return (
                                    <button
                                        ...
                                        className={cn(
                                            "flex flex-col items-center justify-center p-2.5 rounded-xl border bg-card transition-all duration-300 hover:scale-[1.03] select-none",
                                            active ? tierMeta.activeClasses : cn("border-border hover:bg-muted/60 hover:border-border", tierMeta.classes)
                                        )}
                                    >
                                        <span className="text-xs font-bold leading-none">
                                            {resolvedLanguage === 'ar' ? tierMeta.labelAr : tierMeta.labelEn}
                                        </span>
                                        <span className={cn("text-[9px] font-medium mt-1 leading-none opacity-80", active ? "text-background" : "text-muted-foreground")}>
                                            {tierMeta.rate}
                                        </span>
                                    </button>
                                );
                                ...
                                <>
                                    ...
                                    <div className="space-y-2">
                                        {(main.subtasks || []).map((sub: any, subIdx: number) => (
                                            <Card key={sub.id || `${mainIdx}-${subIdx}`} className="py-0 gap-0 bg-muted/20">
                                                <CardContent className="p-3 space-y-3">
                                                    <Input
                                                        value={sub.task}
