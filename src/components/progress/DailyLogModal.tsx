...
export default function DailyLogModal({ goal, tasks, onClose, onSuccess, language = 'ar' }: DailyLogModalProps) {
            ...
            <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                <div className="bg-card rounded-3xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-300 border border-border">
                    <div className="p-8 text-center space-y-6">
                        <div className="w-20 h-20 bg-primary/12 text-primary rounded-full flex items-center justify-center mx-auto mb-2 animate-bounce border border-primary/25">
                        ...
                        <div className="bg-primary/12 p-6 rounded-2xl border border-primary/25 text-start">
                            ...
                            {Array.isArray(evaluation.main_breakdown) && evaluation.main_breakdown.length > 0 ? (
                                    ...
                                    <div key={mbIdx} className="space-y-1">
                                        <div className="flex justify-between items-center text-sm py-1.5">
                                            <span className="font-bold text-foreground/90 truncate flex-1 text-start flex items-center gap-1.5">
                                                <span className={`w-2 h-2 rounded-full shrink-0 ${mb.status === 'done' ? 'bg-primary' : mb.status === 'partial' ? 'bg-foreground/60' : 'bg-muted-foreground/20'}`} />
                                                {mb.main_task}
                                            </span>
                                        ...
                                        {(evaluation.subtask_breakdown || evaluation.task_breakdown || [])
                                            ...
                                            .map((item: any, idx: number) => (
                                                <div key={idx} className="flex justify-between items-center text-xs py-1 ps-4 border-s-2 border-border/70">
                                                    <span className="text-foreground/75 truncate flex-1 text-start">
                                ...
                                (evaluation.subtask_breakdown || evaluation.task_breakdown || []).map((item: any, idx: number) => (
                                    <div key={idx} className="flex justify-between items-center text-sm py-2 border-b border-border last:border-0">
                                        <span className="text-foreground/75 font-medium truncate flex-1 text-start">
                                            {tasks.find((tk: any) => tk.id === item.task_id)?.task_description || t.generalProgress}
                                        </span>
    ...
    return (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
            <div className="bg-card rounded-t-3xl sm:rounded-3xl w-full max-w-lg overflow-hidden animate-in slide-in-from-bottom duration-300 border border-border">
                ...
                {/* IN-APP NOTIFICATION */}
                {notification && (
                    <div className={`mx-6 mt-4 p-3 rounded-xl flex items-center gap-3 ${notification.type === 'error' ? 'bg-destructive/12 border border-destructive/25 text-destructive' : 'bg-primary/12 border border-primary/25 text-primary'
                        }`}>
