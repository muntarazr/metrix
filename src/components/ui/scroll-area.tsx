  ...
  return (
    ...
    >
      <ScrollAreaPrimitive.Viewport
        // Radix renders a `display:table; min-width:100%` wrapper inside the
        // viewport so horizontal content can define the scroll width. In a
        // vertical-only area that box shrink-wraps to its content instead of
        // the viewport, so `truncate`/`min-w-0` children have no definite width
        // to resolve against and blow the row out sideways. globals.css flips
        // that wrapper back to a block when this attribute is present.
        data-vertical-only={showHorizontalScrollbar ? undefined : ""}
        className={cn(
